...
describe("markdown editor actions", () => {
  it("exposes edit/read state through a shadcn choice group and saves with a Button", () => {
    const onSave = vi.fn();
    render(<MarkdownEditor value="처음 메모" onSave={onSave} />);

    const read = screen.getByRole("radio", { name: "읽기" });
    ...
    expect(screen.getByRole("radio", { name: "읽기" })).toHaveAttribute("aria-checked", "true");
  });
});
