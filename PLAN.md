...
- `apps/web/src/components/system/` sits beside `ui/` and contains all Mungchilog wrappers, product classes, and map-specific adaptations. Application screens import these wrappers instead of importing `ui/` directly.
- Shared product tokens remain in the application stylesheet. The wrapper layer maps them to shadcn component states without replacing the established light, dark, typography, and mobile behavior.
...
2. **Product wrapper layer**
   - Create `AppButton`, `MapIconButton`, `AppTooltip`, `AppPopover`, `AppMenu`, `AppSheet`, `AppDialog`, `AppTabs`, and `AppSwitch` under `components/system`.
   - Keep focus, disabled, loading, keyboard, and touch behavior in the wrapper contract.
   - Add product-specific classes only in the wrapper layer.
