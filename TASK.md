...
(현재 없음 — 이전에 있던 "장소 검색 자동완성"은 M2에서 완료됨)
...
## v1에서 뺀 것 (하지 않음)
TSP 자동 최적화, 예산 정산, 사진 업로드, 항공/숙박 예약 파싱, Electron 데스크톱 셸, 브라우저 푸시·이메일 알림, 실시간 커서, CRDT 기반 동시편집. OIDC와 여행별 게스트·편집자 공유는 M6으로 v1에 포함한다.
